 ...
 */
export function resolveResearchTopic(
  state: Pick<OrchestratorState, "message" | "slots" | "research_summary" | "research_package" | "recent_messages">
): string {
  const slot = typeof state.slots.topic === "string" ? state.slots.topic.trim() : "";
  if (slot) return slot;

  const fromSummary = typeof state.research_summary?.topic === "string" ? state.research_summary.topic.trim() : "";
  if (fromSummary && !HITL_MSG.test(fromSummary)) return fromSummary;

  const fromPkg = typeof state.research_package?.topic === "string" ? state.research_package.topic.trim() : "";
