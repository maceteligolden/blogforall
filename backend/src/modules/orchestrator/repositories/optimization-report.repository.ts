...
import { contentOptimizationReportSchema, type ContentOptimizationReport } from "../ai/contracts/content-optimization";
...
export class OptimizationReportRepository {
  ...
  async findLatestForDraft(workspaceId: string, draftId: string): Promise<ContentOptimizationReport | null> {
    const doc = await OptimizationReportModel.findOne({
      workspace_id: workspaceId,
      draft_id: draftId,
    })
