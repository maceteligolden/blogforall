...
export function IllustrationPlan({ className = "", animated = true }: { className?: string; animated?: boolean }) {
  return (
    <IllustrationShell className={className} animated={animated}>
      <svg viewBox="0 0 240 176" className="h-full w-full" fill="none" aria-hidden>
        ...
        <rect
          ...
          height="28"
          rx="10"
          className="fill-white/[0.08] stroke-white/12"
          strokeWidth="1"
        />
        ...
        {[0, 1, 2, 3].map((i) => (
          <g key={i} transform={`translate(0 ${48 + i * 26})`}>
            ...
            {i < 3 && <path d="M160 7 V19" className="stroke-primary/35" strokeWidth="1.2" strokeDasharray="2 3" />}
            <rect
              x="174"
              y="-4"
              width={i === 1 ? 40 : 48}
              ...
              rx="1.5"
              className={cn(
                i < 2 ? "fill-white/30" : "fill-white/15",
                animated && i === 1 && "motion-safe:animate-landing-type-sweep"
              )}
            />
