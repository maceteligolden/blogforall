...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    ...
    void startWritingThread({
      blog_id: id,
      campaign_id:
        typeof (blog as { campaign_id?: string }).campaign_id === "string"
  ...
  const isDirty = savedFingerprint != null && formFingerprint(compareFields) !== savedFingerprint;
  ...
  const handleReview = async () => {
    const useBlocks = Array.isArray(formData.content_blocks) && formData.content_blocks.length > 0;
    const contentForReview = useBlocks ? blocksToHtml(formData.content_blocks as ContentBlock[]) : formData.content;

    if (!formData.title || !contentForReview || !contentForReview.trim()) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateBlog.mutate(
      ...
      {
        onError: (error: Error) => {
          const apiMessage = (error as Error & { response?: { data?: { message?: string } } }).response?.data?.message;
        ...
        right={
          <div className="h-full min-h-0 overflow-y-auto flex flex-col">
            <div className="px-6 lg:px-8 py-6 flex-shrink-0">
              <Breadcrumb
                items={[{ label: "Posts", href: "/dashboard/posts" }, { label: blog?.title || "Edit Post" }]}
              />
                            ...
                            {((blog as { version_history?: { version: number; created_at: Date }[] })?.version_history
                              ...
                              <div className="border-t border-gray-800 pt-4">
                                ...
                                <div className="space-y-2 max-h-40 overflow-y-auto">
                                  {[
                                    ...(blog as { version_history?: { version: number; created_at: Date }[] })
                                      .version_history!,
                                  ]
