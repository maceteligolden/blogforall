...
export default function BlogNewScheduledPostPage() {
  ...
  useEffect(() => {
    if (selectedCampaign && campaignsResponse) {
      const campaign = campaignsResponse.find((c: { _id: string }) => c._id === selectedCampaign);
      if (campaign) {
        setFormData((prev) => ({
          ...prev,
          campaign_id: selectedCampaign,
          timezone: (campaign as { timezone?: string }).timezone || prev.timezone,
          metadata: {
            campaign_goal: (campaign as { goal?: string }).goal,
            target_audience: (campaign as { target_audience?: string }).target_audience,
          },
        }));
      }
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedCampaign && campaignsResponse) {
      const campaign = campaignsResponse.find((c: { _id: string }) => c._id === selectedCampaign);
      if (campaign) {
        const campaignStart = new Date((campaign as { start_date: string }).start_date);
        const campaignEnd = new Date((campaign as { end_date: string }).end_date);
  ...
  const handleBlogSelect = (blogId: string) => {
    const selectedBlog = blogs?.find((b: { _id: string }) => b._id === blogId);
    setFormData({
      ...formData,
      blog_id: blogId,
  ...
  if (selectedCampaign && campaignsResponse) {
    const campaign = campaignsResponse.find((c: { _id: string }) => c._id === selectedCampaign);
    if (campaign) {
      minDate = new Date((campaign as { start_date: string }).start_date).toISOString().slice(0, 16);
      maxDate = new Date((campaign as { end_date: string }).end_date).toISOString().slice(0, 16);
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            >
              ...
              {campaignsResponse?.map((campaign: { _id: string; name: string }) => (
